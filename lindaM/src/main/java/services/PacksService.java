...
public class PacksService implements IService<Packs> {
    ...
    public void add(Packs packs) throws SQLException {
        String req = "INSERT INTO packs(id_pack,nom,description,prix,type) VALUES(?,?,?,?,?)" ;
        PreparedStatement stmt = connection.prepareStatement(req);
        stmt.setInt(1,packs.getId_Pack());
        stmt.setString(2, packs.getNom());
        stmt.setString(3, packs.getDescription());
        stmt.setFloat(4, packs.getPrix());
        stmt.setString(5, packs.getType());
    ...
    public void update(Packs packs) {
        try {
            String req = "update packs set nom=?,description=? ,prix=? ,type=?where id_pack=?";
            PreparedStatement pst = connection.prepareStatement(req);
            pst.setInt(1, packs.getId_Pack());
    ...
    @Override
    public Packs readById(int id_pack) {
        String requete = "select * from packs where id_pack "+ id_pack;
        Packs packs = null;
        try {
